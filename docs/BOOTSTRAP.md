...
- 补充：bootstrap v0.13 测试已验证 stage3 LLVM IR 可链接运行并 emit stage4 LLVM IR（`/tmp/kooixc_stage4_stage1_compiler.ll`）；同时记录每一阶段 IR 的 bytes + fnv1a64 指纹，作为后续 deterministic build 的追踪信号。
