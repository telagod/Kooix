...
- 可选复用可观测：`bootstrap-heavy` 会记录 `reuse_stage3/reuse_stage2` 命中情况与 bootstrap 日志（`/tmp/bootstrap-heavy-bootstrap.log`），并写入 summary + artifact；并记录 `stage1/compiler` smoke 是否启用。
