...
资源观测（每步耗时 + max RSS + exit code；module preflight 额外输出 `module_preflight_ok/errors/warnings/first_diagnostic`）：
...
- `bootstrap_v0_13.sh` 默认包含 module-aware preflight（`check-modules examples/import_variant_main.kooix`）；若仅排查资源问题可临时传 `KX_MODULE_PREFLIGHT=0` / `KX_HEAVY_MODULE_PREFLIGHT=0`，但建议保持开启以覆盖 namespace/import 回归。重载链路会同步记录 preflight 结果（`ok/errors/warnings/first_diagnostic`），用于定位失败根因。
