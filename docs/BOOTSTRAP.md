...
- 补充：Stage1 侧新增 Kooix 实现的 include loader：`stage1/source_map.kooix:s1_load_source_map`（基于 `host_read_file` 扫描并递归展开顶层 `import "path";`），并将 `stage1/compiler_main.kooix` 的入口加载从 `host_load_source_map` 切换为该实现，以减少对“host 级复合 intrinsics”的依赖。
