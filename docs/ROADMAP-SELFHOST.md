...
- P3（自举能力）继续扩面 `dist/kooixc1` 的真实负载编译：
  ...
  - 补充（2026-02-12）：`bootstrap_v0_13.sh` 默认新增 module-aware preflight（`check-modules examples/import_variant_main.kooix`，可通过 `KX_MODULE_PREFLIGHT=0` 暂停）；`bootstrap-heavy` metrics/summary 同步输出 preflight 开关、耗时与 RSS，并补全 `ok/errors/warnings/first_diagnostic` 诊断摘要；CI artifact 同步打包 preflight JSON（`/tmp/kx-module-preflight-*.json`），并新增 `scripts/bootstrap_module_preflight_json_check.sh` 做指标形态回归校验（含主 `ci` workflow 的 enabled/disabled smoke）。另补 `scripts/bootstrap_ci_sanity.sh`（本地一键串行自检）与 `scripts/bootstrap_ci_sanity_smoke.sh`（CI 同款轻量 mock smoke）。
...
- P9（已完成）schema migration window 回归门禁：
...
- ✅ DoD13：`ci` 与 `bootstrap-heavy` 的 triage summary 字段命名统一为 `state/schema/phase/log`（其中 `schema/phase` 当前固定 `n/a`）。
- ✅ DoD14：`Module-check summary` 的 `pass/warn/error` 三类 case 输出统一为 `state/schema/phase/log`（strict 结果并入 `log`）。
- 验证命令（2026-03-01）：
  - `./scripts/check_json_schema_drift_triage_smoke.sh`
...
- ✅ DoD4：`ast` / `hir` 在“全 namespace import”场景切到 module-aware entry projection，避免 include-style 合并触发 duplicate symbol 阻塞调试输出。
  - 当前行为：projection 会注入 imported stub（函数签名与类型完整，函数体为空），用于保证 AST/HIR 观察链路可用；`run/native` 仍保持原语义链路，后续再收敛。
...
- ✅ DoD13：补齐 namespace import 下“导入泛型函数”执行边界回归：
  ...
  - 新增 `cli_run_tests` 覆盖 `native --run` 对 imported generic function 显式 type args 调用可执行（`Lib::id<Int>(...)`）。
- ✅ DoD14：在 `mir/native` 链路落地“显式 type args 最小单态化”：
  - 在 `lower_hir` 前新增 generic specialization 预处理：按调用点收集 `fn<T>` 实例，生成无泛型实例函数（`<name>__inst_<n>`）并重写调用目标。
  - 仅对“调用点显式 type args”收敛，不扩展隐式推断规则；保持现有语义边界可控。
- 验证命令（2026-03-02）：
  ...
  - `cargo run -p kooixc -- hir examples/import_alias_main.kooix`
  - `cargo run -p kooixc -- mir examples/import_alias_main.kooix`
  - `cargo run -p kooixc -- llvm examples/import_alias_main.kooix`
