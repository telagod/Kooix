...
- 已实现：`fn ... { ... }`、`let`/assignment/`return`、基础表达式（literal/path/call/record literal/成员投影 `x.y`/`if/else`/`while`/`+`/`==`/`!=`）与返回类型静态校验。
- 已实现（runtime 起步）：最小 interpreter，可 `run` 纯函数体子集（禁用 effects）。
- 未实现：`enum/match`、模块系统/多文件编译、可自举所需的 runtime/stdlib，以及更完整的执行语义（VM/bytecode 或真正 lowering）。
