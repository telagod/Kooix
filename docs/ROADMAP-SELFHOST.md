...
- P4（下一刀）推进 `dist/kooixc1` 的编译器本体负载：
  ...
  - ✅ DoD5：新增可选 import namespace gate：`bootstrap-heavy` 在 gate1 可通过 `KX_HEAVY_IMPORT_SMOKE=1`（或 dispatch `run_import_smoke=true`）覆盖 `import "x" as Foo; Foo::bar`，并导出对应 IR/二进制 artifact。
