...
- 已实现（去 host 耦合一步）：Stage1 侧新增 Kooix include loader：`stage1/source_map.kooix:s1_load_source_map`（基于 `host_read_file`），并将 `stage1/compiler_main.kooix` 的入口加载切换到该实现（不再依赖 `host_load_source_map`）。
...
- 把 `dist/kooixc1` 作为“日常编译器”扩面：用它编译+链接更接近真实负载的目标（逐步从 `stage2_min` 扩到 `lexer/parser/typecheck` 等），并把 smoke 固化为脚本门禁（默认不开深链路，避免资源打满）。
- 补齐写编译器必需的 stdlib/runtime（最小集合优先）：更可控的 `Text` builder、`List` 工具、再到 `Map`（符号表/环境）；逐步减少 host-only intrinsics 的耦合。
- 设计并推进从 include-style `import "path";` 进化到最小 module/namespace/export（让 Stage1 能长期维护而不爆炸）。
