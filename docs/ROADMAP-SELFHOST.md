...
- 已实现（bootstrap v0.13+ 强化）：Stage1 self-host driver 现在会直接链接产出 stage2 compiler binary（`host_link_llvm_ir_file`），减少对 `kooixc0 native-llvm` 的依赖；并提供一键产物脚本 `./scripts/bootstrap_v0_13.sh` 生成 `dist/kooixc1`（stage3 compiler binary，可用于编译+链接 Kooix 程序）。CI/测试包含可复现信号门禁（stage2/3/4/5 IR 指纹一致 + 可选 golden/determinism/deep）。
...
- P3（自举能力）继续扩面 `dist/kooixc1` 的真实负载编译：
  ...
  - ✅ DoD2：资源可控链路已验证（默认轻量链路；`CARGO_BUILD_JOBS=1` 可单线程限载；`KX_REUSE_STAGE3=1`/`KX_REUSE_STAGE2=1` 可跳过重复重建；`KX_REUSE_ONLY=1` 可在复用缺失时快速失败；深链路由 `KX_DEEP=1` 按需开启；`KX_*` 开关按布尔解析，`0/false` 不会误触发重载 smoke）。
  ...
  - 验证命令（2026-02-12）：`CARGO_BUILD_JOBS=1 KX_SMOKE_IMPORT=1 ./scripts/bootstrap_v0_13.sh`（覆盖 import loader + namespace 调用 `import "x" as Foo; Foo::bar`，含 `examples/import_alias_main` 与 `stage1/stage2_import_alias_smoke`）。
- P4（下一刀）推进 `dist/kooixc1` 的编译器本体负载：
  ...
  - ✅ DoD2：已把“真实负载 smoke”纳入可选 CI gate：新增 `bootstrap-heavy` workflow（`workflow_dispatch` + nightly `schedule`，调用 `scripts/bootstrap_heavy_gate.sh` 低资源运行；dispatch 可选 `run_determinism`/`run_deep`/`run_compiler_smoke`/`run_selfhost_eq`/`reuse_stage3`/`reuse_stage2`/`reuse_only`）。
