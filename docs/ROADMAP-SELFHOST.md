...
- P3（自举能力）继续扩面 `dist/kooixc1` 的真实负载编译：
  ...
  - ✅ DoD2：资源可控链路已升级为“默认硬约束”模式：`bootstrap_v0_13.sh` 默认 `KX_SAFE_MODE=1`（强制 `CARGO_BUILD_JOBS=1`、默认优先复用 stage3/stage2、命令级 timeout、可选 `KX_SAFE_MAX_VMEM_KB`/`KX_SAFE_MAX_PROCS` 限载）；`bootstrap_heavy_gate.sh` 默认同步启用 `KX_HEAVY_SAFE_MODE=1` 并对 gate2/gate3 增加 timeout + max RSS 采样；两条链路均输出资源观测文件（`/tmp/kx-bootstrap-resource.log`、`/tmp/bootstrap-heavy-metrics.txt`）。
