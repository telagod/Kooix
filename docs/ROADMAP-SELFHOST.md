...
- P3（自举能力）继续扩面 `dist/kooixc1` 的真实负载编译：
  ...
  - 补充（2026-02-12）：`bootstrap_v0_13.sh` 默认新增 module-aware preflight（`check-modules examples/import_variant_main.kooix`，可通过 `KX_MODULE_PREFLIGHT=0` 暂停）；`bootstrap-heavy` metrics/summary 同步输出 preflight 开关、耗时与 RSS。
